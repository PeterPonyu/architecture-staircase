...
.fp_keep_tag <- function(x) {
  ...
  if (length(m) && nzchar(m)) sub("\\s*$", "", m) else NULL
}
...
# backslash and print the macro literally. Consequence: on the tikz tier the
# panel tag is no longer bold. Spacing correctness outweighs that, and captions
# reference the tags as plain "(a)".
...
.fp_text_titles_elem <- function(plot) {
  ...
  plot$theme$plot.title <- ggplot2::element_text(
    ...
    margin     = el$margin,
    face       = if (is.null(el$face)) NULL else el$face)
  plot
}
